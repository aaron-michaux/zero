...
class ExecutionContext
{
 ...
 public:
   ExecutionContext(std::size_t thread_pool_size = 0)
   ...
   {
      assert(!is_running());
      for(std::size_t i = 0; i < size_; ++i) pool_.emplace_back([this]() { io_context_.run(); });
   }
   ...
   ExecutionBroker get_executor() const { return ExecutionBroker{io_context_}; }
