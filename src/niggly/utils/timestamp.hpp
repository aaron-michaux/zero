 ...
 public:
   ...
   static constexpr Timestamp parse(std::string_view s)
   {
      ...
      for(auto i = 0u; i < s.size(); ++i) {
         if(i == 4 or i == 7) {
            if(s[i] != '-') throw std::invalid_argument("unexpected character");
         } else if(i == 13 or i == 16) {
            if(s[i] != '-' and s[i] != ':') throw std::invalid_argument("unexpected character");
         } else if(i == 10) {
      ...
      auto d = [&](unsigned ind) -> int {
         ...
         if(!std::isdigit(s[ind])) throw std::invalid_argument("unexpected character");
         return int(s[ind] - '0');
      };
   ...
   ///@{ @name Setters
   void set_micros(int micros)
   {
      assert(micros >= 0 && micros < M);
      x_ = value_type(seconds_from_epoch()) * M + micros;
   }

   void set_seconds_to_epoch(int64_t s) { x_ = value_type(s) * M + value_type(micros()); }

   void set(int64_t s, int micros)
   {
      set_seconds_to_epoch(s);
      set_micros(micros);
   }

   void set(int y, int m, int d, int hour = 0, int min = 0, int sec = 0, int micros = 0)
   {
