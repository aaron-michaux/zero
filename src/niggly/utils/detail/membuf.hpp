 ...
 protected:
                    ...
                    std::ios_base::openmode which) override
   {
      auto do_seek = [&](char_type* base,
         ...
         char_type* ptr     = (dir == std::ios_base::beg)   ? base
                              : (dir == std::ios_base::cur) ? p
                                                            : end;
      ...
      if(which & std::ios_base::out) {
         abs_pos
             = do_seek(this->pbase(), this->pptr(), this->epptr(), std::ios_base::out);
      }

      if(which & std::ios_base::in) {
         abs_pos = do_seek(this->eback(), this->gptr(), this->egptr(), std::ios_base::in);
      }
      return abs_pos;
   }
   ...
   pos_type seekpos(pos_type off, std::ios_base::openmode which) override
   {
      ...
      if(which & std::ios_base::in)
         this->setg(this->eback(), this->eback() + ptrdiff_t(pos), this->egptr());
      return pos;
   }
