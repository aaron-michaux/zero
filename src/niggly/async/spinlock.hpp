...
public:
  ...
  bool try_lock() {
    ...
    return !std::atomic_exchange_explicit(&padlock_, true, std::memory_order_acq_rel);
  }

  /**
   * @brief Unlocks. Non-blocking.
   */
  void unlock() { std::atomic_store_explicit(&padlock_, false, std::memory_order_release); }
   ...
   */
  bool is_locked() const // For assert only!
  {
