...
namespace detail
{
   ...
   struct expected_storage_base
   {
      ...
      template<class... Args,
               detail::enable_if_t<
                   std::is_constructible<E, Args&&...>::value>* = nullptr>
   ...
   // This specialization is for when both `T` and `E` are
   // trivially-destructible, so the destructor of the `expected` can be
   // trivial.
   ...
   struct expected_operations_base : expected_storage_base<T, E>
   {
      ...
      // implementation is used while maintaining the strong exception
      // guarantee. The problematic case is where rhs has a value, but *this
      // does not.
      //
   ...
   template<class T,
            class E,
            bool
            = is_void_or<T, TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>::
   ...
   // This specialization is for when T or E are not trivially copy
   // constructible
   template<class T, class E>
...
// move assignable
   ...
   // This is needed to be able to construct the expected_default_ctor_base
   // which follows, while still conditionally deleting the default constructor.
   ...
   struct expected_default_ctor_base
   {
      ...
      constexpr expected_default_ctor_base(
          ...
          = default;
      constexpr expected_default_ctor_base(
          expected_default_ctor_base&&) noexcept
          = default;
                 ...
                 private detail::expected_default_ctor_base<T, E>
{
 ...
 public:
...
#if defined(TL_EXPECTED_CXX14) && !defined(TL_EXPECTED_GCC49) \
    && !defined(TL_EXPECTED_GCC54) && !defined(TL_EXPECTED_GCC55)
   /// \brief Carries out some operation on the stored unexpected object if
   /// there is one. \returns Let `U` be the result of
   /// `std::invoke(std::forward<F>(f), value())`. If `U` is `void`, returns an
   /// `expected<T,monostate>`, otherwise returns an `expected<T,U>`. If `*this`
   /// has an expected value, the result is `*this`, otherwise an
   /// `expected<T,U>` is constructed from
   /// `make_unexpected(std::invoke(std::forward<F>(f), value()))` and is
   /// returned.
   ///
   /// \group map_error
   ...
   /// \brief Carries out some operation on the stored unexpected object if
   /// there is one. \returns Let `U` be the result of
   /// `std::invoke(std::forward<F>(f), value())`. Returns an `expected<T,U>`.
   /// If `*this` has an expected value, the result is `*this`, otherwise an
   /// `expected<T,U>` is constructed from
   /// `make_unexpected(std::invoke(std::forward<F>(f), value()))` and is
   /// returned.
   ///
   ...
   template<
       class G = E,
       detail::enable_if_t<
           std::is_constructible<E, const G&>::value>*                = nullptr,
   ...
   template<class U,
            ...
            detail::expected_enable_from_other<T,
                                               E,
                                               U,
                                               G,
                                               const U&,
   ...
   template<
       ...
       class G = T,
       detail::enable_if_t<
           std::is_nothrow_constructible<T, U&&>::value>* = nullptr,
   ...
   template<class... Args,
            detail::enable_if_t<
                std::is_nothrow_constructible<T, Args&&...>::value>* = nullptr>
...
namespace detail
{
   ...
   template<class Exp,
            ...
            class Ret
            = decltype(detail::invoke(std::declval<F>(), *std::declval<Exp>()))>
